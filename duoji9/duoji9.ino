...
{
//舵机2由下向上
   val2=1800-300*j;//1800微妙相当于117度，注意舵机二的舵机不能设置不能大于2000，因为以及快挨近舵机1，同时舵机经计算需要让舵机在正负30度范围内为好因此在1200-1800微妙内合适
    angle=(val2-500.0)/2000.0*180.0;//将pwm占空比换算为角度
    for(int i=0;i<=50;i++) //产生PWM个数，等效延时以保证能转到响应角度
    {
      servopulse2(servopin2,val2);//模拟产生PWM
...
for(int i=0;i<3;i++)
{
    ...
    val1=1200+300*i;//1200相当于63度，1500相当于90度，1800微妙相当于117度，注意舵机二的舵机不能设置不能大于2000，因为以及快挨近舵机1，同时舵机经计算需要让舵机在正负30度范围内为好因此在1200-1800微妙内合适
    angle=(val1-500.0)/2000.0*180.0;//将pwm占空比换算为角度
    for(int i=0;i<=50;i++) //产生PWM个数，等效延时以保证能转到响应角度
    {
      servopulse1(servopin1,val1);//模拟产生PWM
